...
const TitleUpdater = () => {
  ...
  useEffect(() => {
    ...
    const isPricingOrCheckout =
      ...
      pathname.startsWith('/vinayaka_chaturthi_offer') ||
      pathname.startsWith('/old_plans') ||
      pathname.includes('checkout') ||
      pathname.startsWith('/12m') ||
      pathname.startsWith('/hdph') ||
    ...
    let newImage = `${window.location.origin}/thumbnails/free-programme.jpg`;
    if (isPricingOrCheckout) {
      if (isUsd) {
        newImage = `${window.location.origin}/thumbnails/usd-pricing.jpg`;
      } else {
        newImage = `${window.location.origin}/thumbnails/inr-pricing.jpg`;
      }
    }

...
const Pmax = ({ defaultLanguage = '' }: PmaxProps) => {
    ...
    const pushFocusEvent = (field: string) => {
        pushDataLayer({ 'event': 'input_focus', 'field_name': field, 'page_name': 'pmax' });
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        pushDataLayer({ 'event': 'form_submit_attempt', 'page_name': 'pmax' });

        if (!validatePhone(formData.phone, formData.dialCode)) {
            setPhoneError(true);
            pushDataLayer({ 'event': 'form_validation_error', 'field': 'phone', 'page_name': 'pmax' });
            return;
        }
        setPhoneError(false);
        ...
        if (!formData.language) {
            ...
            pushDataLayer({ 'event': 'form_validation_error', 'field': 'language', 'page_name': 'pmax' });
            return;
        }
        ...
        if (
            ...
            pushDataLayer({ 'event': 'registration_self_referral', 'page_name': 'pmax' });
        ...
        try {
            ...
            const source = searchParams.get('source') || searchParams.get('ref') || 'website_pmax';
            ...
            if (response.ok || response.status === 409) {
                ...
                // Determine Pmax popup_id based on status and language
                ...
                if (formData.language === 'Telugu') {
                    currentPopupId = isFreeAgain ? 2330 : isNewReg ? 2317 : 2336;
                } else {
                    currentPopupId = isFreeAgain ? 2331 : isNewReg ? 2589 : 2592;
                }
                ...
                if (isNewReg) {
                    pushDataLayer({ 'event': 'registration_success', 'page_name': 'pmax', 'language': formData.language, 'status': resolvedStatus });
                } else if (isFreeAgain) {
                    pushDataLayer({ 'event': 'registration_free_eligible_again', 'page_name': 'pmax', 'language': formData.language });
                } else {
                    pushDataLayer({ 'event': 'registration_duplicate', 'page_name': 'pmax', 'status': resolvedStatus });
                }
                ...
                pushDataLayer({ 'event': 'registration_api_error', 'page_name': 'pmax', 'http_status': response.status });
                console.error('Registration failed');
                alert(`Registration failed: ${data.message || 'Please try again.'}`);
            }
        } catch (error) {
            pushDataLayer({ 'event': 'registration_network_error', 'page_name': 'pmax' });
                    ...
                    <div className="w-full lg:w-1/2 flex flex-col justify-center items-center gap-[13px] px-4 py-[20px] lg:py-[52px] order-2 lg:order-none">
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-4 px-2 pb-4 lg:pb-0">
                            ...
                            <div className="h-[160px] md:h-[201px] w-[calc(50%-8px)] flex flex-col justify-center items-center gap-2.5 bg-[#0d468b] p-4 md:p-5 rounded-2xl shadow-lg">
                                ...
                                <span className="font- text-[16px] leading-[25px] text-center text-white">
                                    13,95,704+ Participated Already
                                </span>
                            </div>
                        </div>
                    </div>
